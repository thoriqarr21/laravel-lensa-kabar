...
<div class="ads" style="padding-top: 0">
    @foreach ($headerads as $item)
        @if (request()->is('/') && $item->type == 'home')
            <img src="{{ asset('images/advertisements/'.$item->sidebar_one) }}" alt="Ads" class="width-100 sidebar-one">
                @elseif ($item->type == 'category')
                @if(request()->path() == 'page/category/'.$item->slug)
                    <img src="{{ asset('images/advertisements/'.$item->sidebar_one) }}" alt="Ads" class="width-100">
                @endif
            @endif                
    @endforeach
</div>
...
<div class="sidebar-item">
    <div class="sidebar-news">
        ...
        <div class="section-item">
            ...
            <h3><a href="{{ route('page.news',$newsinRandomOrder->slug) }}">{{ $newsinRandomOrder->title }}</a></h3>
            <p>{{ $newsinRandomOrder->excerpt }}
                <a href="{{ route('page.news',$newsinRandomOrder->slug) }}"class="btn btn-primary">Read more</a>
            </p>
            <ul>
