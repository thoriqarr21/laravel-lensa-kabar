...
class FrontController extends Controller
{
    ...
    {
        $topnewslist = News::latest()->whereHas('category')->where('status',1)->take(9)->get();

        $newscategory_one   = News::latest()->whereHas('category')->where('category_id',6)->where('status',1)->take(9)->get();
        $newscategory_two   = News::latest()->whereHas('category')->where('category_id',7)->where('status',1)->take(6)->get();
        $newscategory_three = News::latest()->whereHas('category')->where('category_id',3)->where('status',1)->take(5)->get();
...
{
    $newscategorylist = Category::where('status', 1)->get();
    ...
    public function pageCategory($slug)
    {
        ...
        $featurednewslist   = $category->newslist()->where('status',1)->where('featured',1)->take(6)->get();
        $newscategorylist   = $category->newslist()->where('status',1)->where('featured',1)->take(8)->get();
